...
def get_favs_csv_stocks_history(list_companys, csv_name = "FAV"):
    ...
    for l in list_companys:
        df_l = yhoo_history_stock.get_json_stock_values_history(l, Option_Historical.MONTH_3, get_technical_data=True,
        ...
        for c in columns_candle:  # a pesar de que no se haya dado ningun patron de vela el Scaler tiene que respetar el mas menos
            df_l.at[0,c] = -100
            df_l.at[1,c] = 100
...
for item in classifiers:
    ...
    valid_string = str(countValidTrue) + "ñ"+ str(len(df_m['counter']))
