# Handover — publicar `puretrade` en PyPI

Documento de traspaso para llevar `puretrade` de "carpeta dentro de otro repo" a
librería publicada e instalable con `pip install puretrade`.

---

## 0. Punto de partida (estado actual)

- **Dónde está el código:** carpeta `puretrade/` dentro del repo
...
- **Qué es:** 160 indicadores técnicos en puro `numpy`+`pandas`, verificados
  contra TA-Lib / py_ti y point-in-time. Docs en `puretrade/docs/`.
...
2. Nombre: **`puretrade`** (owner `Leci37`).
3. **Sin** README, **sin** .gitignore, **sin** licencia (para no chocar con la
   historia que vamos a empujar).
4. Crear. Anota la URL: `https://github.com/Leci37/puretrade.git`.
...
## 3. Paso 2 — Extraer `puretrade` con su historia y subirlo

Desde la raíz del repo actual (`TensorFlow-stocks-…`):

```bash
# 1) extrae la historia de la carpeta puretrade/ a una rama (queda en la RAÍZ)
...
python -c "import puretrade as pt; print(len(pt.catalog()))"   # ~330
```

Al estar `puretrade` en la raíz del repo nuevo, `.github/workflows/ci.yml` y
...
1. En PyPI: crea el proyecto o ve a *Your projects* → **Publishing** → *Add a
   pending publisher*:
   - PyPI Project Name: `puretrade`
   - Owner: `Leci37`  ·  Repository: `puretrade`
...
pip install --index-url https://test.pypi.org/simple/ puretrade   # smoke test
...
```bash
cd /tmp && python -m venv v && v/bin/pip install puretrade
v/bin/python -c "import puretrade as pt; print(pt.__version__, len(pt.catalog()))"
```

Comprueba también la ficha pública: `https://pypi.org/project/puretrade/`.
...
| El workflow de release no arranca | El tag debe empezar por `v` (`v0.1.0`). Y `puretrade` debe estar en la raíz del repo nuevo. |
