...
for S in list_companys:
    ...
    for time_op in ALL_TIME_OPTIONS:
        ...
        time_series_data = json_data.get(f"Time Series ({intelval})")

        if time_series_data is None:
            print("No time series data received, continuing to the next company.")
            break

        df_a_time = pd.DataFrame.from_dict(time_series_data, orient='index')
        df_a_time.reset_index(inplace=True)
        df_a_time.rename(
        ...
        if df_S_all.shape[0] == 0:
            print("No data collected for", S)
            continue

        print("Column Names:", df_S_all.columns)  # Debug: Print column names
        print("Sample Row:", df_S_all.iloc[0])  # Debug: Print sample row

        if 'Date' in df_S_all.columns:
            df_S_all['Date'] = pd.to_datetime(df_S_all['Date'])
            df_S_all = df_S_all.sort_values(by='Date', ascending=True)
        else:
        ...
        print(S + " df: " + str(df_a_time.shape)  )
        df_S_all = pd.concat([df_S_all, df_a_time], ignore_index=False)
        print("Column Names:", df_S_all.columns)
        print("Sample Row:", df_S_all.iloc[0])
