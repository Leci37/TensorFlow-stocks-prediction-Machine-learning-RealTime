...
from a_manage_stocks_dict import MODEL_TF_DENSE_TYPE_MULTI_DIMENSI, MODEL_TF_DENSE_TYPE_ONE_DIMENSI
...
class ModelDefinition:
    ...
    def __model_time_multi(self):
        ...
        model = Sequential()
        # add model layers
        model.add(TimeDistributed(Conv1D(128, kernel_size=1, activation='relu',  input_shape=(self.shape_inputs[0],self.shape_inputs[1],1 ) )))
        ...
        return self.__compiler_multi_dimensions(model).build(input_shape=(self.shape_inputs[0],self.shape_inputs[1],1 ))
